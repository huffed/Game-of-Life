...
try:
    ...
    raise NotImplementedError(
        '"{}" is currently not supported.'.format(platform.system())) from e
...
def wcswidth(text: str) -> int:
    if not hasattr(wcswidth, "libc"):
        if platform.system() == "Darwin":
            wcswidth.libc = ctypes.cdll.LoadLibrary(
                "libSystem.dylib")  # type: ignore
        elif WINDOWS:
            pass
        else:
            wcswidth.libc = ctypes.cdll.LoadLibrary(
                "libc.so.6")  # type: ignore
    ...
    return wcswidth.libc.wcswidth(  # type: ignore
        ctypes.c_wchar_p(encoded_text.decode(
            encoding=user_locale)), len(encoded_text)
    )
...
class TerminalMenu:
    ...
    class Selection:
        def __init__(self, num_menu_entries: int, preselected_indices: Optional[Iterable[int]] = None):
            ...
            self._selected_menu_indices = set(
                preselected_indices) if preselected_indices is not None else set()
        ...
        def decrement_active_index(self) -> None:
            if self._active_displayed_index is not None:
                if self._active_displayed_index > 0:
                    ...
                    self._active_displayed_index = len(
                        self._displayed_index_to_menu_index) - 1
                self._viewport.keep_visible(self._active_displayed_index)
    ...
    class Viewport:
        def __init__(
            ...
            self._viewport = (
                0, min(self._num_menu_entries, self._num_lines) - 1)
            self.keep_visible(cursor_position=None,
                              refresh_terminal_size=False)
        ...
        def keep_visible(self, cursor_position: Optional[int], refresh_terminal_size: bool = True) -> None:
            ...
            self._viewport = (
                self._viewport[0] + scroll_num, self._viewport[1] + scroll_num)
    ...
    if WINDOWS:
        ...
        _codename_to_capname = {
            ...
            "fg_cyan": f"{CSI}36m",
            "fg_gray": f"{CSI}37m",
            # BOLD - Italic not supported, to my knowledge
            "italics": f"{CSI}1m",
    ...
    def __init__(
        ...
        *,
        ...
        search_highlight_style: Optional[Iterable[str]
                                         ] = DEFAULT_SEARCH_HIGHLIGHT_STYLE,
        search_key: Optional[str] = DEFAULT_SEARCH_KEY,
        shortcut_brackets_highlight_style: Optional[Iterable[str]
                                                    ] = DEFAULT_SHORTCUT_BRACKETS_HIGHLIGHT_STYLE,
        shortcut_key_highlight_style: Optional[Iterable[str]
        ...
        status_bar: Optional[Union[str, Iterable[str],
                ...
                if isinstance(item, int):
                    ...
                    try:
                        ...
                        raise UnknownMenuEntryError(
                            'Pre-selection "{}" is not a valid menu entry.'.format(item)) from e
                else:
                    raise ValueError(
        ...
        self._menu_cursor_style = tuple(
            menu_cursor_style) if menu_cursor_style is not None else ()
        self._menu_highlight_style = tuple(
            menu_highlight_style) if menu_highlight_style is not None else ()
        self._multi_select = multi_select
        self._multi_select_cursor = multi_select_cursor
        self._multi_select_cursor_brackets_style = (
            tuple(
                multi_select_cursor_brackets_style) if multi_select_cursor_brackets_style is not None else ()
        )
        self._multi_select_cursor_style = (
            tuple(
                multi_select_cursor_style) if multi_select_cursor_style is not None else ()
        )
        self._multi_select_keys = tuple(
            multi_select_keys) if multi_select_keys is not None else ()
        ...
        self._preselected_indices = (
            convert_preselected_entries_to_indices(
                preselected_entries) if preselected_entries is not None else None
        )
        ...
        self._search_highlight_style = tuple(
            search_highlight_style) if search_highlight_style is not None else ()
        self._search_key = search_key
        self._shortcut_brackets_highlight_style = (
            tuple(
                shortcut_brackets_highlight_style) if shortcut_brackets_highlight_style is not None else ()
        )
        self._shortcut_key_highlight_style = (
            tuple(
                shortcut_key_highlight_style) if shortcut_key_highlight_style is not None else ()
        )
        self._show_search_hint = show_search_hint
        ...
        self._status_bar_style = tuple(
            status_bar_style) if status_bar_style is not None else ()
        ...
        self._view = self.View(self._menu_entries, self._search,
                               self._selection, self._viewport, self._cycle_cursor)
    ...
    def _init_term(self) -> None:
        ...
        if WINDOWS:
            self._tty_in = open(sys.stdin.fileno(), "r",
                                encoding=self._user_locale)
            self._tty_out = open(sys.stdout.fileno(), "w",
            ...
            self._new_term[3] = cast(
                int, self._new_term[3]) & ~termios.ICANON & ~termios.ECHO & ~termios.ICRNL
            self._new_term[0] = cast(int, self._new_term[0]) & ~termios.ICRNL
            termios.tcsetattr(
                self._tty_in.fileno(), termios.TCSAFLUSH, cast(
                    List[Union[int, List[Union[bytes, int]]]], self._new_term)
            )
        ...
        if self._clear_screen:
            ...
            if WINDOWS:
                ...
                self._tty_out.write(self._get_terminal_size_windows()[
                                    1] * self._codename_to_terminal_code["cursor_up"])
    ...
    def _reset_term(self) -> None:
        ...
        if not WINDOWS:
            ...
            termios.tcsetattr(
                self._tty_out.fileno(), termios.TCSAFLUSH, cast(
                    List[Union[int, List[Union[bytes, int]]]], self._old_term)
            )
    ...
    def _paint_menu(self) -> None:
            ...
            if self._status_bar_func is not None and self._view.active_menu_index is not None:
                status_bar_lines = tuple(
                    self._status_bar_func(
                        self._menu_entries[self._view.active_menu_index]).strip().split("\n")
                )
        ...
        def print_menu_entries() -> int:
            ...
            for displayed_index, menu_index, menu_entry in self._view:
                ...
                if self._search and self._search.search_text != "":
                    ...
                    self._tty_out.write(
                        menu_entry[: min(
                            match_obj.start(), num_cols - all_cursors_width - shortcut_string_len)]
                    )
                    ...
                    if menu_index == self._view.active_menu_index:
                        apply_style(self._menu_highlight_style)
                    self._tty_out.write(
                        menu_entry[match_obj.end(): num_cols -
                                   all_cursors_width - shortcut_string_len]
                    )
                else:
                    self._tty_out.write(
            ...
            self._tty_out.write("\r" + (self._viewport.size - 1)
                                * self._codename_to_terminal_code["cursor_up"])
        ...
        def print_preview(current_menu_height: int, preview_max_num_lines: int) -> int:
                ...
                if isinstance(self._preview_command, str):
                    try:
                        preview_process = subprocess.Popen(
                            [cmd_part.format(preview_argument) for cmd_part in shlex.split(
                        ...
                        raise PreviewCommandFailedError(
                            e.stderr.decode(
                                encoding=self._user_locale, errors="replace").strip()
                        ) from e
                else:
            ...
            def strip_ansi_codes_except_styling(string: str) -> str:
                stripped_string = strip_ansi_codes_except_styling.ansi_escape_regex.sub(  # type: ignore
                    ...
                    # type: ignore
                    if strip_ansi_codes_except_styling.ansi_sgr_regex.match(match_obj.group(0))
                    else "",
                    string,
                )
            ...
            try:
                ...
                preview_string = "The preview command failed with error message:\n\n" + \
                    str(e)
            self._tty_out.write(current_menu_height *
                    ...
                    self._tty_out.write(
                        (
                            ...
                            + (2 * BoxDrawingCharacters.horizontal +
                               " " + self._preview_title)[: num_cols - 3]
                            + " "
                    ...
                    self._tty_out.write(
                        (
                            ((BoxDrawingCharacters.vertical + " ")
                             if self._preview_border else "")
                            + limited_line
                            + self._codename_to_terminal_code["reset_attributes"]
                            + max(num_cols - limited_line_len -
                                  (3 if self._preview_border else 0), 0) * " "
        ...
        def delete_old_menu_lines(displayed_menu_height: int) -> None:
            ...
            if (
                ...
                self._tty_out.write((displayed_menu_height + 1)
                                    * self._codename_to_terminal_code["cursor_up"])
        ...
        def position_cursor() -> None:
            ...
            for displayed_index in range(self._viewport.lower_index, self._viewport.upper_index + 1):
                ...
                if displayed_index < self._viewport.upper_index:
                    self._tty_out.write(
                        self._codename_to_terminal_code["cursor_down"])
            self._tty_out.write((self._viewport.size - 1)
                                * self._codename_to_terminal_code["cursor_up"])
        ...
        def print_multi_select_column() -> None:
            ...
            def prepare_multi_select_cursors() -> Tuple[str, str]:
                ...
                apply_style(self._multi_select_cursor_brackets_style,
    ...
    def _clear_menu(self) -> None:
        ...
        if self._clear_menu_on_exit:
            if self._title_lines:
                self._tty_out.write(len(self._title_lines) *
                                    self._codename_to_terminal_code["cursor_up"])
                self._tty_out.write(
    ...
    def chosen_menu_entries(self) -> Optional[Tuple[str, ...]]:
        return (
            tuple(self._menu_entries[menu_index]
...
""",
    ...
    parser.add_argument("-C", "--no-cycle", action="store_false",
                        dest="cycle", help="do not cycle the menu selection")
    parser.add_argument(
        "-E",
        "--no-exit-on-shortcut",
        action="store_false",
    ...
    parser.add_argument("-t", "--title", action="store",
                        dest="title", help="menu title")
    parser.add_argument(
        "-V", "--version", action="store_true", dest="print_version", help="print the version number and exit"
    )
    parser.add_argument("entries", action="store", nargs="*",
                        help="the menu entries to show")
...
def parse_arguments() -> AttributeDict:
    ...
    if args.search_highlight_style != "":
        args.search_highlight_style = tuple(
            args.search_highlight_style.split(","))
    else:
